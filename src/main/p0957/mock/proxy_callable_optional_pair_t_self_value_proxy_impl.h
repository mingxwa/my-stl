...
#ifndef SRC_MAIN_P0957_MOCK_PROXY_CALLABLE_OPTIONAL_PAIR_T_SELF_VALUE_PROXY_IMPL_H_
#define SRC_MAIN_P0957_MOCK_PROXY_CALLABLE_OPTIONAL_PAIR_T_SELF_VALUE_PROXY_IMPL_H_
 ...
 * facade Callable<optional<pair<First, self_value_proxy>>>()> {
 *   optional<pair<First, proxy<Callable<optional<pair<First, self_value_proxy>>()>>>> operator()();
 * };
 */
...
struct facade_meta_t<Callable<optional<pair<First, self_value_proxy>>()>, E> {
 ...
 private:
  template <class T>
  static optional<pair<First, value_proxy<Callable<optional<pair<First, self_value_proxy>>()>>>> callable_op_0(E<false, false> erased) {
    return erased.cast(in_place_type<T>)();
  }

  optional<pair<First, value_proxy<Callable<optional<pair<First, self_value_proxy>>()>>>> (*callable_op_0_)(E<false, false>);
};

template <class First, class A>
class proxy<Callable<optional<pair<First, self_value_proxy>>()>, A> : public A {
 public:
  proxy() : A() {}
  ...
  optional<pair<First, value_proxy<Callable<optional<pair<First, self_value_proxy>>()>>>> operator()() {
...
#endif  // SRC_MAIN_P0957_MOCK_PROXY_CALLABLE_OPTIONAL_PAIR_T_SELF_VALUE_PROXY_IMPL_H_
